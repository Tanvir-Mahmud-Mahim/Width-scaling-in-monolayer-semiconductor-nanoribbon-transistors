...
strain-charge plane: the sloping solid lines are the $A_1'$ constraint and the
vertical dashed lines the 2LA(M) constraint, each in the colour of the location
it belongs to. They intersect at finite carrier density and zero strain for the
edge, and at finite strain and zero charge for an interior inhomogeneity. (d) The edge charge produces a width-dependent threshold shift,
...
parameters from frozen-phonon calculations under biaxial strain, the $E'$ values
from a symmetric displacement and the $A_1'$ values from a relaxed scan of the
chalcogen height, compared with direct measurements. (b) Computed strain derivative of the
...
representative growth technologies, the two atomic-layer-deposited and the
sputtered values inverted here from published Raman ratios and the remaining
three taken from the literature. (c) Screening of the four
...
measured monolayer MoS$_2$ saturation velocity,\cite{Smithe2018} and solve the
channel self-consistently: the surface band bending and the electron
quasi-Fermi potential are carried as coupled fields, so that the quantum
capacitance, the fall of the channel charge towards the drain and the feedback
of a series contact resistance all enter without approximation. The coupled
system is solved by Newton's method in the open-source device simulator
DEVSIM,\cite{Sanchez2022} with the edge charge entering through
Eq.~(\ref{eq:dvt}) and the damage halo through the width-averaged mobility.
Taking $V_{\rm ov}=\Vov$~V, $V_{\rm DS}=1$~V and a 300~nm channel to match the
measured devices, this gives \IonScMoSTwentyfive\ and
\IonScMoSSeventyfive~$\mu$A/$\mu$m for MoS$_2$ ribbons of width 25 and 75~nm on
the high-$\kappa$ stack, against 310 and 400~$\mu$A/$\mu$m measured, and
\IonScWSTwo~$\mu$A/$\mu$m for WS$_2$ at 43~nm against 420~$\mu$A/$\mu$m
(Fig.~\ref{fig:edge}(c)).\cite{Pena2026} The model therefore reproduces the
measured width dependence and the ordering of the materials while sitting
between 21 and 38\%\ below the absolute level, which is what a semiclassical
model carrying no fitted transport parameter can be expected to do.

The p-type WSe$_2$ device is the informative exception. The channel model
predicts \IonScWSeTwo~$\mu$A/$\mu$m against 130~$\mu$A/$\mu$m measured, and
reproducing the measurement requires a contact resistance of
\RcWSeTwo~k$\Omega\,\mu$m per contact, about \RcRatio\ times the
\RcMeas~$\Omega\,\mu$m reported for the n-type devices of the same
work.\cite{Pena2026} That is the
known difficulty of making a p-type contact to
WSe$_2$,\cite{Sun2026,Mondal2024} and the self-consistent solve separates it
from the channel rather than leaving it as an unquantified caveat.
...
defect potential and the interface charge density of each gate stack. Currents
quoted against measurement come from a self-consistent Poisson and
drift-diffusion solve that includes contact resistance. The design map and the
critical-width scans use the faster compact expression, which overestimates the
absolute current by a nearly constant factor, the self-consistent to compact
ratio being \ScRatio; the critical width, being a ratio of currents, differs
between the two models by only \WcModelDiff\%. The solve is semiclassical, so quantum transport is not
treated and short-channel electrostatics enter only through the assumed channel
length.
